...
def encode(prompt, add_special_tokens=True, add_bos_token=True, truncation_length=None):
    # encode aramus prompt tokens
    if shared.model.__class__.__name__ in ['AramusModel']:
        input_ids = prompt
        return input_ids
...
def _generate_reply(question, state, stopping_strings=None, is_chat=False):
    ...
    if generate_func is None:
        ...
        elif shared.model.__class__.__name__ in ['AramusModel']:
            generate_func = generate_reply_aramus
        else:
...
def generate_reply_aramus(question, original_question, seed, state, stopping_strings=None, is_chat=False):
    ...
    try:
        ...
        original_tokens=0
        new_tokens=0
        print(f'Output generated by aramus model in {(t1-t0):.2f} seconds ({new_tokens/(t1-t0):.2f} tokens/s, {new_tokens} tokens, context {original_tokens}, seed {seed})')
        return